    ...
    final @Nullable Bundle state)
  {
    ...
    final AsyncTask<Void, Void, Void> task =
      new AsyncTask<Void, Void, Void>() {
        @Override protected @Nullable Void doInBackground(
          final @Nullable Void... params)
        {
          DRMTestActivity.LOG.debug("task started");

          try {
            final AdobeAdeptConnectorFactoryType f =
              AdobeAdeptConnectorFactory.get();
            final AdobeAdeptNetProviderType net =
              AdobeAdeptNetProvider.get("Simplified");

            final AdobeAdeptConnectorType p =
              f.get(
                "org.nypl.simplified.app",
                DRMTestActivity.this,
                net,
    ...
    DRMTestActivity.LOG.debug("starting task");
    task.execute();
  }
