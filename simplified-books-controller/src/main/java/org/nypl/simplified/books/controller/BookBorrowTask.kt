...
import org.nypl.simplified.books.controller.api.BookBorrowExceptionFetchingACSMFailed
import org.nypl.simplified.books.controller.api.BookBorrowExceptionFetchingBookFailed
...
class BookBorrowTask(
  ...
  @Volatile
  private lateinit var bookInitial: org.nypl.simplified.books.api.Book
  ...
  private fun runAcquisitionBorrow(): Unit {
    ...
    return when (feedResult) {
      is FeedLoaderResult.FeedLoaderSuccess -> {
        val resultFeed = feedResult.feed
        when (resultFeed) {
          is Feed.FeedWithoutGroups -> {
            val entries =
              checkFeedHasEntries(this.acquisition.uri, resultFeed.entriesInOrder)
          ...
          is Feed.FeedWithGroups -> {
            val groups =
              checkFeedHasGroups(this.acquisition.uri, resultFeed.feedGroupsInOrder)
            val entries =
  ...
  private fun getRequiredAccountCredentials(): org.nypl.simplified.accounts.api.AccountAuthenticationCredentials {
  ...
  @Throws(IOException::class, org.nypl.simplified.books.book_database.api.BookDatabaseException::class)
  private fun runAcquisitionBundled(): Unit {
    ...
    try {
      ...
      throw e
    } catch (e: org.nypl.simplified.books.book_database.api.BookDatabaseException) {
      FileUtilities.fileDelete(file)
      throw e
    }
  }
