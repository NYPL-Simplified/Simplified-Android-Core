...
import org.nypl.simplified.profiles.controller.api.AccountDeleteErrorDetails
...
class ProfileAccountDeleteTask(
  ...
  : Callable<AccountDeleteTaskResult> {
  ...
  private fun publishFailureEvent(step: TaskStep<AccountDeleteErrorDetails>) =
    this.accountEvents.send(AccountEventDeletion.AccountEventDeletionFailed(step.resolution))
  ...
  override fun call(): AccountDeleteTaskResult {
    return try {
      ...
      this.publishFailureEvent(
        this.taskRecorder.currentStepFailed(this.strings.onlyOneAccountRemaining, null, e))
      AccountDeleteTaskResult(this.taskRecorder.finish())
    } catch (e: Throwable) {
      this.publishFailureEvent(
        this.taskRecorder.currentStepFailed(this.strings.deletionFailed, null, e))
      AccountDeleteTaskResult(this.taskRecorder.finish())
    }
  }
}
