...
public final class OPDSJSONSerializer implements OPDSJSONSerializerType
{
    ...
    final OPDSAvailabilityType av)
  {
    ...
    final ObjectMapper jom = new ObjectMapper();
    return av
      .matchAvailability(new OPDSAvailabilityMatcherType<ObjectNode, UnreachableCodeException>() {
          ...
          final OPDSAvailabilityLoaned a)
        {
          ...
          a.getEndDate().map(new FunctionType<Calendar, Unit>() {
    ...
    throws OPDSFeedSerializationException
  {
    ...
    e.getCover().map(new FunctionType<URI, Unit>() {
      @Override public Unit call(
        final URI u)
      {
    ...
    e.getPublished().map(new FunctionType<Calendar, Unit>() {
      @Override public Unit call(
        final Calendar c)
      {
    ...
    e.getPublisher().map(new FunctionType<String, Unit>() {
      @Override public Unit call(
        final String s)
      {
        je.put("publisher", s);
    ...
    e.getThumbnail().map(new FunctionType<URI, Unit>() {
      @Override public Unit call(
        final URI u)
      {
    ...
    throws OPDSFeedSerializationException
  {
    ...
    e.getFeedNext().map(new FunctionType<URI, Unit>() {
      @Override public Unit call(
        final URI next)
      {
    ...
    e.getFeedSearchURI().map(new FunctionType<OPDSSearchLink, Unit>() {
      @Override public Unit call(
