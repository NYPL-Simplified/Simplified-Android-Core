...
open class BookmarksUpdateTaskContract {
  ...
  /**
   * Trying to update bookmarks for an EPUB works.
   */
  ...
  fun testUpdateEPUB() {
    ...
    val task =
      BookmarksUpdateTask(
        account = account,
        bookID = bookID,
        bookmarks = { Bookmarks(lastRead = bookmark0, bookmarks = bookmarks0) })
